    ...
    if(backendType_ == IPSERVER) {
        if(backendIP_ == "127.0.0.1") {
            ...
            if(!IPCopyFile_(backendIP_, backendDirName_ + shortFileName, "", cacheDirName_ + shortFileName)) {
                fprintf(stderr,
                        "Error: fail to store the file '%s' from the local cache '%s' to the backend storage '%s' with IP '%s'!\n",
    ...
    if(backendType_ == IPSERVER) {
        if(backendIP_ == "127.0.0.1") {
            if(!localCopyFile_(cacheDirName_ + shortFileName, backendDirName_ + shortFileName)) {
                fprintf(stderr,
                        "Error: fail to restore the file '%s' from the backend storage '%s' to the cache '%s'!\n",
            ...
            if(!IPCopyFile_("", cacheDirName_ + shortFileName, backendIP_, backendDirName_ + shortFileName)) {
                fprintf(stderr,
                        "Error: fail to restore the file '%s' from the backend storage '%s' with IP '%s' to the local cache '%s'!\n",
