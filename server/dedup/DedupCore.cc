...
bool DedupCore::addPrefixDir_(const std::string &prefixDirName, std::string &name)
{
    if((name.substr(0, 1) == "/") || (name.substr(0, 2) == "./") || (name.substr(0, 3) == "../")) {
        fprintf(stderr,
                "Error: cannot add a prefix dir for the name ('%s'), since it begins with '/', './', or '../'!\n",
                name.c_str());
        return 0;
    }
                                         ...
                                         CryptoPrimitive *cryptoObj)
{
    ...
    returnValue = cryptoObj->generateHash(hashInputBuffer, fullFileName.size() + sizeof(int),
                                          (unsigned char *) inodeFP);
...
bool DedupCore::flushBufferNodeIntoDisk_(perUserBufferNode_t *targetBufferNode)
{
    ...
    if(targetBufferNode->recipeFileBufferCurrLen > 0) {
        ...
        if(targetBufferNode->lastRecipeHeadPos == 0) {
            ...
            /*open a recipe file for appending recipes; if it does not exist, then create it*/
            ...
            if(fileSize == 0) {
                ...
                if(fwrite(targetBufferNode->recipeFileBuffer, targetBufferNode->recipeFileBufferCurrLen, 1, fp) != 1) {
                    fprintf(stderr, "<flushBufferNodeIntoDisk_1> Error: fail to write recipes into the file '%s'!\n",
                            recipeFileName.c_str());
                ...
                fseek(fp,
                      recipeFileOffset + fileRecipeHeadSize_ + fileRecipeEntrySize_ * (InsFileRecipeHead.numOfShares),
                      SEEK_SET);
            ...
            if(fwrite(targetBufferNode->recipeFileBuffer, targetBufferNode->recipeFileBufferCurrLen, 1, fp) != 1) {
                fprintf(stderr, "<flushBufferNodeIntoDisk_2>Error: fail to write recipes into the file '%s'!\n",
                        recipeFileName.c_str());
    ...
    if(fileStat.IsNotFound()) {
        ...
        while(!dirName.empty()) {
            ...
            if(dirStat.IsNotFound()) {
                ...
                pInodeIndexValueHead = (inodeIndexValueHead_t *) (value + valueOffset);
                pInodeIndexValueHead->userID = userID;
                pInodeIndexValueHead->shortNameSize =
                        shortName.size() + 1; /*size() do not consider the null-terminator*/
...
bool DedupCore::storeNewRecipeFile_(perUserBufferNode_t *targetBufferNode, std::string &recipeFileName)
{
    ...
    /*write the data of the recipe file buffer into a recipe file*/
    if(fwrite(targetBufferNode->recipeFileBuffer, targetBufferNode->recipeFileBufferCurrLen, 1, fp) != 1) {
        fprintf(stderr, "<storeNewRecipeFile_>Error: fail to write recipes into the file '%s'!\n",
                recipeFileName.c_str());
        fprintf(stderr, "<storeNewRecipeFile_>length is: %d\n\n", targetBufferNode->recipeFileBufferCurrLen);
                                   ...
                                   int &recipeFileOffset)
{
    ...
    if(getStat.IsNotFound()) {
        fprintf(stderr, "<findOldRecipeFile_>Error: cannot find an inode for the key '%s' in the database!\n",
                keySlice->ToString().c_str());
...
bool DedupCore::appendOldRecipeFile_(perUserBufferNode_t *targetBufferNode, std::string &recipeFileName)
{
    ...
    /*find the old recipe file info in the database*/
    if(!findOldRecipeFile_(targetBufferNode, recipeFileName, recipeFileOffset)) {
        fprintf(stderr, "<appendOldRecipeFile_>Error: fail to find the old recipe file info in the database!\n");
        return 0;
    }
                                          ...
                                          unsigned char *recipeFileBuffer)
{
    ...
    if((targetBufferNode != NULL) && (strcmp(targetBufferNode->recipeFileName, recipeFileName) == 0)) {
        ...
        printf("[Meta] <readRecipeFileFromBuffer_> get recipe file from buffer successfully\n");
        return 1;
    } else {
        /*release the mutex lock bufferLock_*/
        pthread_mutex_unlock(&bufferLock_);
        printf("[Meta] <readRecipeFileFromBuffer_> recipe file not stored in buffer \n");
                                 ...
                                 unsigned char *shareDataBuffer, CryptoPrimitive *cryptoObj, bool end)
{
    ...
    while(shareMDBufferOffset < shareMDSize) {
        ...
        if(pFileShareMDHead->numOfPastSecrets == 0) {
            recipeFileBufferAddedLen =
                    fileRecipeHeadSize_ + fileRecipeEntrySize_ * (pFileShareMDHead->numOfComingSecrets);
        }
        ...
        if(targetBufferNode->recipeFileBufferCurrLen + recipeFileBufferAddedLen > RECIPE_BUFFER_SIZE) {
            printf("[Meta] <secondStage> Buffer size limit reached! Store recipe buffer into disk...\n");
            ...
            if((targetBufferNode->lastRecipeHeadPos == 0) &&
               (pFileShareMDHead->numOfPastSecrets > pFileRecipeHead->numOfShares)) {
                ...
                printf("[Meta] <secondStage> store recipe buffer into disk...\n");
                if(!appendOldRecipeFile_(targetBufferNode, recipeFileName)) {
                    fprintf(stderr,
                            "<secondStageDedup>Error: fail to append the data of the recipe file buffer to a previous recipe file!\n");

                    return 0;
                }
            } else {
                printf("[Meta] <secondStage> store new recipe file...\n");
                if(!storeNewRecipeFile_(targetBufferNode, recipeFileName)) {
                    fprintf(stderr,
                            "Error: fail to store the data of the recipe file buffer into a new recipe file!\n");
        ...
        for(i = 0; i < pFileShareMDHead->numOfComingSecrets; i++) {
            ...
            if(intraUserDupStatList[numOfShares] != 1) {
                ...
                if(memcmp(pShareMDEntry->shareFP, shareFP, FP_SIZE) != 0) {
                    fprintf(stderr,
                            "Error: the %d-th share and its fingerprint sent by userID '%d' are inconsistent!\n", i,
                            userID);
                ...
                if(!interUserIndexUpdate_(shareFP, userID, pShareMDEntry->shareSize, targetBufferNode, shareDataBuffer,
                    ...
                    fprintf(stderr,
                            "Error: fail to update the share index for inter-user duplication in the database!\n");
            ...
            // file recipes of metadata chunk
            ...
            pFileRecipeEntry->segID = pShareMDEntry->segID;
            pFileRecipeEntry->shareID = pShareMDEntry->shareID;
        ...
        if(end) {
            printf("[Meta] <secondStage> {finish} End hit\n");
            ...
            if((targetBufferNode->lastRecipeHeadPos == 0) &&
               (pFileShareMDHead->numOfPastSecrets > pFileRecipeHead->numOfShares)) {
                if(!appendOldRecipeFile_(targetBufferNode, recipeFileName)) {
                    fprintf(stderr,
                            "Error: fail to append the data of the recipe file buffer to a previous recipe file!\n");

                    return 0;
                }
                printf("[Meta] <secondStage> {finish} appended old recipe file...\n");

                if(recipeStorerObj_ != NULL) {
                    recipeStorerObj_->addNewFile(recipeFileName);
                    printf("[Meta] <secondStage> {finish} added new recipe file...\n");
                }
            } else {
                /*do nothing*/
                printf("[Meta] <secondStage> {finish} no need to do anything. It's ok...\n");
            }
        }
    }
...
/*
 * restore a share file for a user and write it into file recipe(of data chunks) for data chunk restoration
 *
 *
 * @param userID - the user id
 ...
 */
bool DedupCore::restoreShareFileAndWriteFileRecipe(const int &userID,
                                                   ...
                                                   int socketFD, CryptoPrimitive *cryptoObj)
{
    ...
    /*if such an inode for fullFileName exists*/
    if(inodeStat.ok()) {
        int shareFileBufferSize = SHARE_FILE_BUFFER_SIZE;
        ...
        shareFileBuffer = (unsigned char *) malloc(shareFileBufferSize);
        shareContainerCache = (shareContainerCacheNode_t *) malloc(
                sizeof(shareContainerCacheNode_t) * NUM_OF_CACHED_CONTAINERS);
        ...
        printf("[Meta] file recipe of metadata -> recipeFileName: %s\n", pInodeFileEntry->recipeFileName);
        /*first read recipe file from the buffer; if it is not in the buffer, then read it from the disk*/
        if(!(recipeFileIsInBuffer = readRecipeFileFromBuffer_(userID, pInodeFileEntry->recipeFileName,
                                                              recipeFileBuffer))) {
        ...
        // write into pShareFileHead <=> write into shareFileBuffer
        pShareFileHead = (shareFileHead_t *) (shareFileBuffer + shareFileBufferOffset);
        pShareFileHead->fileSize = pFileRecipeHead->fileSize;
        long fileSize = pShareFileHead->fileSize;
        printf("[Meta] <restore> file size = %ld\n", pShareFileHead->fileSize);
        if(fileSize < 0) {
            printf("[Meta] <restore> file size < 0!! Interal error!!!\n");
            return false;
        }
        pShareFileHead->numOfShares = pFileRecipeHead->numOfShares;
        printf("[Meta] <meta> share number = %d\n", pShareFileHead->numOfShares);
        shareFileBufferOffset += shareFileHeadSize_;

        FILE *fpFileRecipeName = fopen(fileRecipeName.c_str(), "wb+");
        if(fpFileRecipeName == nullptr) {
            printf("can't open recipe file %s\n", fileRecipeName.c_str());
            return false;
        } else {
            printf("[!>] [Meta] open recipe file %s successfully!!\n", fileRecipeName.c_str());
        }
        int numOfDataShares = 0;

        /* skip fileRecipeHead_t */
        fseek(fpFileRecipeName, sizeof(fileRecipeHead_t), SEEK_SET);

        int fileSizeCounter = 0;

        // metaListBuffer for storing metalist and use this buffer to send back to client
        auto metaListBuffer = std::make_unique<unsigned char[]>(METALIST_BUFFER_SIZE);
        ...
        for(i = 0; i < numOfShares; i++) {
            ...
            if(recipeFileBufferOffset + fileRecipeEntrySize_ > RECIPE_BUFFER_SIZE) {
                printf("[Meta] <restore> recipe buffer size limit reached!!\n");
                if(recipeFileIsInBuffer) {
                    ...
                    if(fread(recipeFileBuffer + recipeFileBufferTailLen, 1, recipeFileBufferOffset,
                             recipeFilePointer) == 0) {
                        fprintf(stderr, "Error: fail to read the recipe file '%s'!\n", fullRecipeFileName.c_str());
            ...
            if(shareStat.ok()) {
                ...
                j = 0;
                while((j < numOfCachedShareContainers) &&
                      (strcmp(pShareIndexValueHead->shareContainerName,
                ...
                if(shareFileBufferOffset + shareMDEntrySize_ + pShareIndexValueHead->shareSize >
                   shareFileBufferSize) {
                    // reset shareFileBufferOffset
                    shareFileBufferOffset = 0;
                }

                /* write file recipe when restoring data */
                /* parse the share object */
                ItemMeta_t output{};
                output.type = 1;

                /*generate and store the share info into shareFileBuffer*/
                // This is the file recipes of metadata chunk.
                pShareMDEntry = (shareMDEntry_t *) (shareFileBuffer + shareFileBufferOffset);

                pShareMDEntry->secretID = pFileRecipeEntry->secretID;
                pShareMDEntry->secretSize = pFileRecipeEntry->secretSize;
                pShareMDEntry->shareSize = pShareIndexValueHead->shareSize;
                pShareMDEntry->segID = pFileRecipeEntry->segID;
                pShareMDEntry->shareID = pFileRecipeEntry->shareID;


                memcpy(&(output.shareObj.share_header), pShareMDEntry, sizeof(shareMDEntry_t));

                shareFileBufferOffset += shareMDEntrySize_;
                ...
                // read share data from meta data chunk and store them into file recipe
                int dataShares = 0;
                memcpy(&dataShares, shareFileBuffer + shareFileBufferOffset, sizeof(int));
                numOfDataShares += dataShares;

                /* skip counter added in client::Encoder::collect */
                shareFileBufferOffset += sizeof(int);

                // output.shareObj.data <-> counter<int> + [metaNode_1, metaNode_2, metaNode_n]
                memcpy(output.shareObj.data, shareFileBuffer + shareFileBufferOffset, pShareMDEntry->shareSize);

                // get the end of secretID in this metadata chunk
                metaNode newNode;
                if(dataShares - 1 < 0) {
                    printf("[Meta] <restore> dataShare - 1 = %d. Internal error.\n", dataShares - 1);
                    return 0;
                }
                memcpy(&newNode, output.shareObj.data + (dataShares - 1) * sizeof(metaNode), sizeof(metaNode));
                // save shorted file recipes(including ID and shareID) as metalist for following uploading
                this->save_as_metalist(metaListBuffer.get(), pShareMDEntry->segID, pShareMDEntry->shareID, i,
                                       newNode.secretID);

                /* write meta-data of data into file recipe(data file recipes) */
                for(int index = 0; index < dataShares; ++index) {

                    memcpy(&newNode, output.shareObj.data + index * sizeof(metaNode), sizeof(metaNode));

                    /* write meta node into file recipe */
                    saveMetaNode2DataFileRecipe(fpFileRecipeName, &newNode, fileSizeCounter, index);
                }
            }
        ...
        // the first int value is the counter(<-> numOfShares)
        int size_meta_list_buffer = sizeof(int) + numOfShares * sizeof(MetaList);
        this->assemble_full_meta_list_buffer(metaListBuffer.get(), numOfShares);
        this->send_meta_list(metaListBuffer.get(), size_meta_list_buffer, numOfShares, socketFD);

        fileRecipeHead_t fileRecipeHeader;
        fileRecipeHeader.userID = userID;
        fileRecipeHeader.fileSize = fileSize;
        fileRecipeHeader.numOfShares = numOfDataShares;
        printf("fileRecipe header: \n");
        printf("\tuserID: %d\n", fileRecipeHeader.userID);
        printf("\tfile size: %ld\n", fileRecipeHeader.fileSize);
        printf("\tnumOfShares: %d\n", fileRecipeHeader.numOfShares);

        /* IF empty data in this server, send indicator to client(downloadFileRecipes) and exit thread */
        if(fileRecipeHeader.numOfShares == 0) {
            printf("[Meta] [restore] Empty meta data chunks in this server. numOfShares = 0\n");
            /* `END_DOWNLOAD_INDICATOR` tells client to end downloading chunk */
            indicator = END_DOWNLOAD_INDICATOR;
            auto buffer = std::make_unique<char[]>(sizeof(int));
            memcpy(buffer.get(), &indicator, sizeof(int));
            int retValue = 0;
            retValue = send(socketFD, buffer.get(), sizeof(int), 0);
            if(retValue != sizeof(int)) {
                fprintf(stderr, "Error informing client to end downloading chunks\n");
                return -1;
            }
            printf("[Meta] [restore] client informed!\n");
            fclose(fpFileRecipeName);
        ...
        /* set position of file to the beginning */
        rewind(fpFileRecipeName);

        printf("[Meta] num of shares: %d\n", fileRecipeHeader.numOfShares);

        // write header into file recipe
        int retValue = -1;
        retValue = fwrite(&fileRecipeHeader, sizeof(fileRecipeHead_t), 1, fpFileRecipeName);
        if(retValue != 1) {
            printf("[Meta] Fails to write file recipe header into %s\n", fileRecipeName.c_str());
            return false;
        }
        printf("[Meta] Write file recipe header into %s\n", fileRecipeName.c_str());

        /* send indicator to client when file recipe is generated */
        sendFileRecipeIndicator(socketFD);
    ...
    if(inodeStat.IsNotFound()) {
        ...
        /* 2. set indicator to `NO_RECIPE_STORED` to tell client that no recipe restored in this server */
        int indicator = NO_RECIPE_STORED; // decrease indicator according to server.hh

        /* 3. send necessary data */
        auto buffer = std::make_unique<char[]>(sizeof(int));
        memcpy(buffer.get(), &indicator, sizeof(int));
        // send indicator
        if((byteCount = send(socketFD, buffer.get(), sizeof(int), 0)) == -1) {
            fprintf(stderr, "Error sending indicator! Error code: %d\n", errno);
            return false;
        }
        printf("[!>] [Meta] inode not find. Indicator '-11' sent\n");
    ...
    this->set_last_share_special_flag(false);
    printf("[Meta] [restore] Exiting restoring function...\n\n");
    delete inodeKeySlice;
 ...
 */
bool DedupCore::saveMetaNode2DataFileRecipe(FILE *fp, metaNode *node, int &fileSizeCounter, int index)
{
    fileRecipeEntry_t writeEntryNode;
    memcpy(writeEntryNode.shareFP, node->shareFP, FP_SIZE);
    // do not need to store share size in file recipe since backend storage engine has that shareSize
    writeEntryNode.secretID = node->secretID;
    writeEntryNode.secretSize = node->secretSize;
    writeEntryNode.segID = node->segID;
    writeEntryNode.shareID = node->shareID;
    fileSizeCounter += node->secretSize;
 ...
 */
bool DedupCore::sendFileRecipeIndicator(int socketFD)
{
    ...
    /* 1. set indicator */
    /* indicator in client is `-111` as well */
    int indicator = FILE_RECIPE_SUCCESS; // decrease indicator according to server.hh
 ...
 */
bool DedupCore::save_as_metalist(unsigned char *meta_list_buffer, int id, int shareID, int write_index,
                                 int end_secretID)
{
    MetaList meta_list{
            id,
    ...
    if(this->last_share_on_server_ && shareID >= NUM_OF_SHARES_NEEDED) {
        // shareID: 0, 1, 2, 3
        // 1. we do not need [ID:3] to restore in order to improve download speed
        // 2. set `shareID = -1` as a placeholder for easy implementation of client-side process
        meta_list.shareID = -1;
    }

    int buffer_offset = sizeof(int) + write_index * sizeof(MetaList);
    if(buffer_offset > METALIST_BUFFER_SIZE) {
        printf("[Meta] [restore] meta_list buffer overflow detected!!\n");
        printf("[Meta] [restore] Consider using bigger size of buffer\n");
        exit(-1);
    }
    memcpy(meta_list_buffer + buffer_offset, &meta_list, sizeof(MetaList));
 ...
 */
void DedupCore::assemble_full_meta_list_buffer(unsigned char *meta_list_buffer, int &count)
{
    // set the `count` in this buffer
    memcpy(meta_list_buffer, &count, sizeof(int));
}
 ...
 * @param socketFD - the file descriptor of the sending socket
 */
bool DedupCore::send_meta_list(unsigned char *metalist_buffer, int total_size, int count, int socketFD)
{

    int byteCount = 0;
    auto buffer = std::make_unique<unsigned char[]>(sizeof(int));

    /* 1. set indicator */
    // start sending meta_list to client
    int indicator = SEND_META_LIST;
    memcpy(buffer.get(), &indicator, sizeof(int));
    // send indicator
    if((byteCount = send(socketFD, buffer.get(), sizeof(int), 0)) == -1) {
        fprintf(stderr, "Error sending indicator! Error code: %d\n", errno);
    ...
    /* 2. set size to be sent */
    // send total_size
    memcpy(buffer.get(), &total_size, sizeof(int));
    if((byteCount = send(socketFD, buffer.get(), sizeof(int), 0)) == -1) {
        fprintf(stderr, "Error sending indicator! Error code: %d\n", errno);
        return false;
    }
    ...
    /* 3. send buffer data */
    // send metalist
    if((byteCount = send(socketFD, metalist_buffer, total_size, 0)) == -1) {
        fprintf(stderr, "Error sending indicator! Error code: %d\n", errno);
        return false;
    }
 ...
 *               Default: false
 */
void DedupCore::set_last_share_special_flag(bool flag)
{
    this->last_share_on_server_ = flag;
}
 ...
 *               and it is no need to send back to client
 */
bool DedupCore::get_last_share_special_flag()
{
    return this->last_share_on_server_;
}
