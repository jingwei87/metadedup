...
class Decoder {
private:
    bool check_all_job_done(const bool *job_done);
...
public:
    ...
    typedef struct {
        ...
        int shareSize;
        int secretID;
        int kShareIDList[NUM_OF_SHARES_NEEDED];
    } ShareChunk_t;

    /* input share buffer */
    MessageQueue<ShareChunk_t> **inputbuffer_;

    /* output secret buffer */
    MessageQueue<Secret_t> **outputbuffer_;
    ...
    /* decode object array */
    CDCodec *decodeObj_[DECODE_NUM_THREADS + 1];
     ...
     * @param kmServerCount - number of KM-assistied server (n - kmServerCount <=> original n)
    ...
    /*
     * set the file size to decoder
     *
     * @param totalFileSize - the total file size of file
     */
    int setFileSize(long totalSecrets);
